...
import { Shield, Target, Users, ArrowRight, Mail, Calendar, MapPin, Globe, ScrollText, FileText, ShoppingBag } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-black text-white selection:bg-white selection:text-black">
        ...
        <div className="container mx-auto max-w-5xl relative z-20 flex flex-col items-center">
          ...
          <div className="grid md:grid-cols-2 gap-6 items-center w-full max-w-4xl">
            ...
            <div className="flex flex-col items-center md:items-start text-center md:text-left space-y-8 bg-neutral-950/90 backdrop-blur-sm border border-neutral-800 text-white p-8 rounded-3xl shadow-2xl shadow-black/50">
              ...
              {/* BUTTONS: LINKED TO HISTORY AND EVENTS */}
              <div className="flex flex-col sm:flex-row gap-4 w-full pt-2">
                <Link href="/history" className="bg-white text-black px-6 py-3 font-bold uppercase tracking-wider hover:bg-neutral-200 transition-all text-center text-sm rounded-full w-full sm:w-auto shadow-lg">
                  About Us
                </Link>
                <Link href="/events" className="border border-neutral-600 text-neutral-300 px-6 py-3 font-bold uppercase tracking-wider hover:border-red-500 hover:text-red-500 transition-all text-center text-sm rounded-full w-full sm:w-auto">
                  Events
                </Link>
              </div>
            </div>
      ...
      {/* 3. FIND A DOJO SECTION: LINKED TO DOJO */}
         ...
         <div className="container mx-auto max-w-5xl">
            <div className="bg-black rounded-[3rem] p-8 md:p-16 border border-neutral-800 relative overflow-hidden flex flex-col md:flex-row items-center gap-12 shadow-2xl">
                ...
                <div className="flex-1 relative z-10 text-center md:text-left flex flex-col items-center md:items-start">
                    ...
                    <Link href="/dojo" className="bg-white text-black hover:bg-neutral-200 px-8 py-4 rounded-full font-bold uppercase tracking-wider transition-colors shadow-lg flex items-center justify-center gap-2 w-fit">
        ...
        <div className="container mx-auto max-w-6xl relative z-10">
           ...
           <div className="grid md:grid-cols-3 gap-8">
              <ResourceCard 
                ...
                title="Black Belt"
                subtitle="Registry"
                desc="The official SKIF-USA Black Belt database—search by name to verify Dan grade and dojo affiliation."
                link="/registry"
              />
              <ResourceCard 
                ...
                title="Technical"
                subtitle="Docs"
                desc="Downloads for grading syllabi, tournament rules, and official instructor manuals."
                link="/technical"
              />
              <ResourceCard 
                ...
                title="Official"
                subtitle="Merch"
                desc="Purchase authentic SKIF-USA stamps, DVDs , books, and accessories."
                link="/shop"
              />
           </div>
        </div>
...
function NewsCard({ category, date, title, location, image, link }: { category: string, date: string, title: string, location: string, image: string, link: string }) {
  return (
    <Link href={link} className="group relative h-[700px] overflow-hidden rounded-3xl border border-neutral-800 bg-neutral-900 cursor-pointer">
      ...
      <div className="absolute inset-0 p-6 flex flex-col justify-end z-20">
         <div className="flex items-end justify-between w-full gap-4">
             ...
             <div className="bg-white text-black p-3 rounded-full shadow-lg shrink-0 opacity-100 translate-y-0 md:opacity-0 md:translate-y-4 md:group-hover:opacity-100 md:group-hover:translate-y-0 transition-all duration-300 ease-out">
                <ArrowRight size={24} />
             </div>
         </div>
      </div>
...
function ResourceCard({ icon, title, subtitle, desc, link }: { icon: any, title: string, subtitle: string, desc: string, link: string }) {
  return (
    <Link href={link} className="group relative h-[350px] bg-neutral-900 border border-neutral-800 hover:border-red-600 rounded-3xl overflow-hidden transition-all duration-500 cursor-pointer flex flex-col p-8 shadow-2xl shadow-neutral-900/20 hover:shadow-red-900/20 hover:-translate-y-2">
        ...
        <div className="flex-1">
            <h3 className="text-3xl font-black uppercase text-white mb-4 leading-none">
                {title} <br/><span className="text-neutral-500 group-hover:text-white transition-colors">{subtitle}</span>
            </h3>
